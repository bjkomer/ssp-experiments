...
limits = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
...
for limit in limits:
    ...
    configs += ssp_configs + pcgauss_configs + legendre_configs + onehot_configs + tilecoding_configs
...
fname_csv = 'noise_resilience_larger_results_{}seeds.csv'.format(len(seeds))
...
if os.path.exists(fname_csv) and not args.overwrite_output:
    ...
    for config in configs:
        ...
        test_limit = min(limit, 16)

        res = int(res_base*test_limit)

        xs = np.linspace(-test_limit, test_limit, res)
        ys = np.linspace(-test_limit, test_limit, res)

        # fixed number of points to test
        xs_coarse = xs[::test_limit]
        ys_coarse = ys[::test_limit]
