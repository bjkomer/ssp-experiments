...
parser = argparse.ArgumentParser('Normalize an SSP in a particular direction')
...
total_samples = args.n_train_samples + args.n_test_samples

input_vectors = np.zeros((total_samples, args.dim))
output_vectors = np.zeros((total_samples, args.dim))

rng = np.random.RandomState(seed=args.seed)
locations = rng.uniform(limit_low, limit_high, size=(total_samples, 2))

for i in range(total_samples):
    input_vectors[i, :] = encoding_func(locations[i, 0], locations[i, 1])
    mag = np.linalg.norm(locations[i, :])
    output_vectors[i, :] = encoding_func(locations[i, 0]/mag, locations[i, 1]/mag)

# output_vectors, input_vectors = generate_direction_dataset(
#     encoding_func,
#     n_samples,
#     args.dim,
#     n_items,
...
train_input = input_vectors[:args.n_train_samples, :]
train_output = output_vectors[:args.n_train_samples, :]
test_input = input_vectors[args.n_train_samples:, :]
test_output = output_vectors[args.n_train_samples:, :]
...
with nengo_dl.Simulator(net, minibatch_size=minibatch_size) as sim:
    ...
    suffix = '{}dim_{}layer_{}_hs{}_{}samples_{}epochs_{}reg'.format(
        ...
        args.n_epochs, args.weight_reg
    )

    param_file = "./saved_params/nengo_direction_params_{}".format(
        suffix
    )
    history_file = "./saved_params/nengo_direction_train_history_{}.npz".format(
        suffix
    )
    nengo_obj_file = "./saved_params/nengo_direction_obj_{}.pkl".format(
        suffix
    )
