...
# ssp prediction for each sensor sample
ssp_outputs = np.zeros((n_mazes_to_use, res, res, repr_dim))
ssp_true = np.zeros((n_mazes_to_use, res, res, repr_dim))

hmv = get_encoding_heatmap_vectors(xs, ys, args.dim, encoding_func, normalize=False)

for bi in range(n_batches):
    print("Maze {} of {}".format(bi + 1, n_batches))
    ...
    sensor_inputs[bi, :, :, :] = vis_input[bi, :, :36*4].reshape(res, res, 36*4)
    ssp_outputs[bi, :, :, :] = batch_data.reshape(res, res, repr_dim)
    ssp_true[bi, :, :, :] = true_ssps.reshape(res, res, repr_dim)

np.savez(
    'spiking_localization_solution.npz',
