...
from datasets import MultiPathDataset
...
parser.add_argument('--n-paths', type=int, default=3, help='number of distinct paths to remember')
parser.add_argument('--batch-size', type=int, default=32)
parser.add_argument('--lr', type=float, default=0.001)
parser.add_argument('--momentum', type=float, default=0.9)
parser.add_argument('--weight-histogram', action='store_true', help='Save histogram of the weights')
parser.add_argument('--path-type', type=str, default='random', choices=['random'], help='path to learn')
parser.add_argument('--logdir', type=str, default='multi_path_function',
...
if args.path_type == 'example':
    ...
    paths = np.random.randint(low=0, high=5, size=(args.n_paths, 6, 6))
else:
...
train_path_indices = np.zeros((args.n_train_samples, 1)).astype(np.int32)  # must be integer to use as index
...
for n in range(args.n_train_samples):
    ...
    path_index = np.random.randint(low=0, high=args.n_paths)

    train_inputs[n, :] = encode_point(x, y, x_axis_sp, y_axis_sp).v
    train_path_indices[n, :] = path_index
    train_coords[n, :] = np.array([x, y])
    train_outputs[n, :] = path_function(train_coords[n, :], paths[path_index, :, :], xs, ys)
...
for n in range(args.n_test_samples):
    ...
    test_path_indices[n, :] = path_index
    test_coords[n, :] = np.array([x, y])
    test_outputs[n, :] = path_function(test_coords[n, :], paths[path_index, :, :], xs, ys)


dataset_train = MultiPathDataset(
    ssp_inputs=train_inputs,
    direction_outputs=train_outputs,
    coord_inputs=train_coords,
    path_indices=train_path_indices,
    n_paths=args.n_paths,
)
dataset_test = MultiPathDataset(
    ssp_inputs=test_inputs,
    direction_outputs=test_outputs,
    coord_inputs=test_coords,
    path_indices=test_path_indices,
    n_paths=args.n_paths,
)
...
# Note that the input is the concatenation of a SSP and a one-hot encoded path index
model = FeedForward(input_size=train_inputs.shape[1] + args.n_paths, output_size=train_outputs.shape[1])
...
with torch.no_grad():
    ...
    if args.logdir != '':
        ...
        for i in range(args.n_paths):
            ...
            # Get only the indices corresponding to the current path of interest
            indices = ssps[:, args.dim + i] == 1

            fig_pred = plot_path_predictions(
                directions=outputs[indices, :], coords=coords[indices, :],
            )
            writer.add_figure('test set predictions path {}'.format(i+1), fig_pred)
            fig_truth = plot_path_predictions(
                directions=directions[indices, :], coords=coords[indices, :],
            )
            writer.add_figure('ground truth path {}'.format(i+1), fig_truth)
