...
for i, classification_dataset in enumerate(datasets):
    ...
    for max_iter in max_iters:
        for hidden_layer_sizes in hidden_layers:
            # contains all results for this iter and number of hidden layers
            df = pd.DataFrame()
            for seed in seeds:
                for scale in scales:
                    for dim in dims:
    ...
    df.to_csv('intermediate/encoding_exp_results_{}.csv'.format(classification_dataset))

    df_all = df_all.append(df, ignore_index=True)

print("Saving All Results")

df_all.to_csv('encoding_exp_all_results.csv')
